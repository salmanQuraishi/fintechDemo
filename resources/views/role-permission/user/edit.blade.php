            ...
            <section class="2xl:flex-1 2xl:mb-0 mb-6">
                ...
                <div class="w-full py-[20px] px-[24px] rounded-lg bg-white dark:bg-darkblack-600">
                    <div class="flex flex-col space-y-5">
                        ...
                        <h4 class="text-lg font-bold text-bgray-900 dark:text-white">Edit User</h4>
                                    ...
                                    <form action="{{ route('users.update',$user->id) }}" method="POST">
                                      ...
                                      <div class="grid 2xl:grid-cols-2 grid-cols-1 gap-6">
                                        <div class="flex flex-col gap-2">
                                          ...
                                          <input type="text" class="bg-bgray-50 dark:bg-darkblack-500 dark:text-white p-4 rounded-lg h-14 border-0 focus:border focus:border-success-300 focus:ring-0" name="name" value="{{ $user->name }}">
                                          <x-input-error :messages="$errors->get('name')" class="mt-2" />
                                        </div>
                                        <div class="flex flex-col gap-2">
                                          <label for="email" class="text-base text-bgray-600 dark:text-bgray-50  font-medium">Email</label>
                                          <input type="email" class="bg-bgray-50 dark:bg-darkblack-500 dark:text-white p-4 rounded-lg h-14 border-0 focus:border focus:border-success-300 focus:ring-0"  name="email" readonly value="{{ $user->email }}" >
                                          <x-input-error :messages="$errors->get('email')" class="mt-2" />
                                        </div>
                                        <div class="flex flex-col gap-2">
                                          <label for="password" class="text-base text-bgray-600 dark:text-bgray-50  font-medium">Password</label>
                                          <input type="password" class="bg-bgray-50 dark:bg-darkblack-500 dark:text-white p-4 rounded-lg h-14 border-0 focus:border focus:border-success-300 focus:ring-0"  name="password">
