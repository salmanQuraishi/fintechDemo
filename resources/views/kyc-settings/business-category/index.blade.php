                ...
                <div class="w-full py-[20px] px-[24px] rounded-lg bg-white dark:bg-darkblack-600">
                    <div class="flex flex-col space-y-5">
                        ...
                        <div class="table-content w-full overflow-x-auto">
                            <table id="bus-cat-table" style="width: 100%">
                                <thead>
            ...
            <div class="step-content step-1 relative">
                <div class="relative max-w-[530px] rounded-lg bg-white dark:bg-darkblack-600 p-7 transition-all">
                    ...
                    <div class="flex flex-col">
                        <div>
                            <form class="space-y-6" id="add-bus-cat-form">
                                <div>
                                    <label for="name"
                                    ...
                                    <input type="text"
                                        ...
                                        id="name" name="name" placeholder="Enter Business Category">
                                </div>
                                <div>
                                    <label for="status"
        ...
        <div class="modal-content w-full max-w-xl px-4 mx-auto">
            <div class="step-content relative">
                <div class="relative max-w-[530px] rounded-lg bg-white dark:bg-darkblack-600 p-7 transition-all">
                    ...
                    <div class="flex flex-col">
                        <div>
                            <form class="space-y-6" id="edit-bus-cat-form">
                                ...
                                <div>
                                    ...
                                    <input type="text"
                                        class="w-full border border-bgray-300 rounded-lg py-3 px-4 focus:border focus:border-success-300 focus:ring-0 dark:bg-darkblack-600 dark:border-darkblack-400 dark:text-white"
                                        id="edit-name" name="name" placeholder="Enter Business Category">
                                </div>
                                <div>
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#bus-cat-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('kyc.business.category.getcat') }}",
                    columns: [
                        {
                            data: null,
                            orderable: false,
                        ...
                        { data: 'status',
                            render: function(data, type, row, meta){
                                return `<label class="switch">
                                            <input type="checkbox" name="status"  class="update-status" data-id="${row.id}"
                                                ${data === 'active' ? 'checked' : ''}>
                ...
                $('#bus-cat-table').on('click', '.edit-token-btn', async function () {
                    const id = $(this).data('id');

                    try {
                        // Fetch business Category data for editing
                        const response = await fetch(`/kyc/business/category/${id}/edit`, {
                            method: 'GET',
                            headers: {
                        ...
                        if (response.ok && data.status === 'success') {
                            ...
                            Toast.fire({
                                icon: 'error',
                                title: data.message || 'Failed to fetch business Category data'
                            });
                        }
                    } catch (error) {
                        console.error(error);
                ...
                $('#add-bus-cat-form').on('submit', async function (e) {
                    e.preventDefault();
                    ...
                    try {
                        const response = await fetch("{{ route('kyc.business.category') }}", {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.status == "success") {
                            $('#multi-step-modal').addClass('hidden');
                            $('#add-bus-cat-form')[0].reset();
                            $('#bus-cat-table').DataTable().ajax.reload();

                            Toast.fire({
                                icon: data.status,
                                title: data.message || 'Business Category saved successfully.'
                            });
                        } else {
                            Toast.fire({
                                icon: data.status,
                ...
                $('#edit-bus-cat-form').on('submit', async function (e) {
                    e.preventDefault();
                    ...
                    try {
                        const response = await fetch("{{ route('kyc.business.category.update') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                $(document).on('click', '.update-status', function () {
                    ...
                    // Send AJAX request
                    fetch('/kyc/business/update-status', {
                        method: 'POST',
                        headers: {
                        'Content-Type': 'application/json',
                    ...
                    .catch(error => {
                        ...
                        // document.querySelector('input[name="status"]').checked = !isActive;
                ...
                // Helper function to get CSRF token from meta tag (if using Laravel or similar frameworks)
