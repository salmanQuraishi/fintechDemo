<x-app-layout>
                ...
                <div class="w-full py-[20px] px-[24px] rounded-lg bg-white dark:bg-darkblack-600">
                    <div class="flex flex-col space-y-5">
                        ...
                        <div class="table-content w-full overflow-x-auto">
                            <table id="bus-sub-cat-table" style="width: 100%">
                                <thead>
            ...
            <div class="step-content step-1 relative">
                <div class="relative max-w-[530px] rounded-lg bg-white dark:bg-darkblack-600 p-7 transition-all">
                    ...
                    <div class="flex flex-col">
                        <div>
                            <form class="space-y-6" id="add-bus-sub-cat-form">
                                <div>
                                    <label for="name"
                                    ...
                                    <input type="text"
                                        ...
                                        id="name" name="name" placeholder="Enter Business Sub Category">
                                </div>
                                <div>
                                    <label for="bus_cat_id"
                                        class="text-sm text-bgray-500 dark:text-bgray-50 block mb-2.5 text-left">Business Category</label>
                                    <select
                                        class="w-full border border-bgray-300 rounded-lg py-3 px-4 focus:border focus:border-success-300 focus:ring-0 dark:bg-darkblack-600 dark:border-darkblack-400 dark:text-white"
                                        id="bus_cat_id" name="bus_cat_id">
    ...
    <div class="modal fixed inset-0 z-50 overflow-y-auto flex items-center justify-center hidden"
        id="edit-business-sub-cat-modal">
        <div class="modal-overlay fixed inset-0 bg-gray-500 opacity-75 dark:bg-bgray-900 dark:opacity-50"></div>
        <div class="modal-content w-full max-w-xl px-4 mx-auto">
            <div class="step-content relative">
                <div class="relative max-w-[530px] rounded-lg bg-white dark:bg-darkblack-600 p-7 transition-all">
                    ...
                    <div class="flex flex-col">
                        <div>
                            <form class="space-y-6" id="edit-bus-sub-cat-form">
                                ...
                                <div>
                                    ...
                                    <input type="text"
                                        class="w-full border border-bgray-300 rounded-lg py-3 px-4 focus:border focus:border-success-300 focus:ring-0 dark:bg-darkblack-600 dark:border-darkblack-400 dark:text-white"
                                        id="edit-name" name="name" placeholder="Enter Business Sub Category">
                                </div>
                                <div>
                                    <label for="edit_bus_cat_id"
                                        class="text-sm text-bgray-500 dark:text-bgray-50 block mb-2.5 text-left">Business Category</label>
                                    <select
                                        class="w-full border border-bgray-300 rounded-lg py-3 px-4 focus:border focus:border-success-300 focus:ring-0 dark:bg-darkblack-600 dark:border-darkblack-400 dark:text-white"
                                        id="edit_bus_cat_id" name="edit_bus_cat_id">
                                        <option value="">Select Business Category</option>
                                        @foreach($businessCategories as $businessCategory)
                                            <option value="{{ $businessCategory->id }}">{{ $businessCategory->name }}</option>
                                        @endforeach
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#bus-sub-cat-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('kyc.business.sub.cat.getcat') }}",
                    columns: [
                        {
                            data: null,
                            orderable: false,
                        ...
                        { data: 'bus_cat_name',
                            render: function (data, type, row, meta) {
                                return `
                                        <span class="short-token">${row.business_category.name}</span>`;
                            }
                        },
                        { data: 'status' },
                        {
                ...
                $('#bus-sub-cat-table').on('click', '.edit-token-btn', async function () {
                    const id = $(this).data('id');

                    try {
                        // Fetch business Category data for editing
                        const response = await fetch(`/kyc/business/sub-cat/${id}/edit`, {
                            method: 'GET',
                            headers: {
                        ...
                        if (response.ok && data.status === 'success') {
                            ...
                            $('#edit_bus_cat_id').val(data.data.bus_cat_id);
                            $('#edit-status').val(data.data.status);

                            // Show the edit modal
                            $('#edit-business-sub-cat-modal').removeClass('hidden');
                        } else {
                            Toast.fire({
                                icon: 'error',
                                title: data.message || 'Failed to fetch business Sub Category data'
                            });
                        }
                    } catch (error) {
                        console.error(error);
                ...
                $('#edit-modal-cancel').on('click', function () {
                    $('#edit-business-sub-cat-modal').addClass('hidden');
                });
                ...
                $('#add-bus-sub-cat-form').on('submit', async function (e) {
                    e.preventDefault();

                    const name = $('#name').val();
                    const bus_cat_id = $('#bus_cat_id').val();
                    const status = $('#status').val();

                    try {
                        const response = await fetch("{{ route('kyc.business.sub.cat') }}", {
                            method: 'POST',
                            headers: {
                            ...
                            body: JSON.stringify({ name,bus_cat_id, status })
                        });
                        ...
                        if (data.status == "success") {
                            $('#multi-step-modal').addClass('hidden');
                            $('#add-bus-sub-cat-form')[0].reset();
                            $('#bus-sub-cat-table').DataTable().ajax.reload();

                            Toast.fire({
                                icon: data.status,
                                title: data.message || 'Business Sub Category saved successfully.'
                            });
                        } else {
                            Toast.fire({
                                icon: data.status,
                ...
                $('#edit-bus-sub-cat-form').on('submit', async function (e) {
                    e.preventDefault();

                    const id = $('#edit-id').val();
                    const name = $('#edit-name').val();
                    const bus_cat_id = $('#edit_bus_cat_id').val();
                    const status = $('#edit-status').val();

                    try {
                        const response = await fetch("{{ route('kyc.business.sub.cat.update') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            ...
                            body: JSON.stringify({ id, name, bus_cat_id, status })
                        });
